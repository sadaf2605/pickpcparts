class ProcessorController < ApplicationController
  ...
  def create
    ...
    @processor_processor.performance = @processor_performance
    @processor_processor.memory = @processor_memory
    @processor_processor.graphic = @processor_graphic
    @processor_processor.advanced_technology = @processor_advanced_technology
    @processor_processor.expansion = @processor_expansion
    @processor_processor.package = @processor_package
    @processor_processor.data_protection = @processor_data_protection
    @processor_processor.platform_protection = @processor_platform_protection
  ...
  def processor_processor_params
    params.require(:processor_processor).permit(:processor_number, :cache_in_MB, :DMI2_in_GTs, :instruction_set_in_bit, :instruction_set_extensions, :embedded_options_available, :lithography_in_nm, :scalability)
  end
  ...
  def processor_graphic_params
    params.require(:processor_graphic).permit(:processor_graphics, :graphics_base_frequency_in_MHz, :graphics_max_dynamic_frequency_GHz, :graphics_video_max_memory_in_GB, :quick_sync_video, :wireless_display, :clear_video_hd_technology, :no_of_display_supported)
  end

  def processor_expansion_params
    params.require(:processor_extension).permit(:PCI_express_revision, :max_no_of_PCI_express_lanes)
  end
